...
test("checkAndGrantAchievements grants streak milestones at correct thresholds", async () => {
  const userId = await createTestUser("streak-milestones");
  ...
  const below = await database.checkAndGrantAchievements(userId, { ...base, streak: 2 }) as Achievement[];
  assert.deepEqual(below, [], "streak of 2 should not unlock any milestone");

  // Streak of 3 — unlocks streak_3
  const at3 = await database.checkAndGrantAchievements(userId, { ...base, streak: 3 }) as Achievement[];
  ...
  const again = await database.checkAndGrantAchievements(userId, { ...base, streak: 3 }) as Achievement[];
  ...
  const at7 = await database.checkAndGrantAchievements(userId, { ...base, streak: 7 }) as Achievement[];
...
test("checkAndGrantAchievements grants XP milestones", async () => {
  const userId = await createTestUser("xp-milestones");
  const base: AchievementCheckInput = {
  ...
  const below = await database.checkAndGrantAchievements(userId, { ...base, totalXp: 50 }) as Achievement[];
  assert.deepEqual(below, []);

  const at100 = await database.checkAndGrantAchievements(userId, { ...base, totalXp: 100 }) as Achievement[];
  assert.equal(at100.length, 1);
  assert.equal(at100[0].id, "xp_100");

  const at500 = await database.checkAndGrantAchievements(userId, { ...base, totalXp: 500 }) as Achievement[];
  ...
  const at5000 = await database.checkAndGrantAchievements(userId, { ...base, totalXp: 5000 }) as Achievement[];
...
test("checkAndGrantAchievements grants category mastery achievement at 80%", async () => {
  const userId = await createTestUser("mastery-80");
  const base: AchievementCheckInput = {
    streak: 0, totalXp: 0, language: "spanish", category: "essentials",
    hintsUsed: 0, revealedAnswers: 0, score: 5, maxScore: 10
  };

  const below = await database.checkAndGrantAchievements(userId, { ...base, mastery: 79 }) as Achievement[];
  assert.deepEqual(below.filter((a) => a.id.startsWith("mastery_")), []);

  const at80 = await database.checkAndGrantAchievements(userId, { ...base, mastery: 80 }) as Achievement[];
  ...
  const again = await database.checkAndGrantAchievements(userId, { ...base, mastery: 95 }) as Achievement[];
  assert.deepEqual(again.filter((a) => a.id === "mastery_spanish_essentials"), []);
});

test("checkAndGrantAchievements does not grant mastery achievement for practice sessions", async () => {
  const userId = await createTestUser("mastery-practice");
  const unlocked = await database.checkAndGrantAchievements(userId, {
    streak: 0, totalXp: 0, language: "spanish", category: "essentials",
...
test("checkAndGrantAchievements grants speed_demon for a perfect no-hint session with 10+ questions", async () => {
  const userId = await createTestUser("speed-demon");
  const base: AchievementCheckInput = {
    streak: 0, totalXp: 0, language: "spanish", category: "essentials",
  ...
  const below = await database.checkAndGrantAchievements(userId, { ...base, score: 9, maxScore: 9 }) as Achievement[];
  assert.deepEqual(below.filter((a) => a.id === "speed_demon"), []);

  // 10 correct, perfect, no hints — unlocks
  const unlocked = await database.checkAndGrantAchievements(userId, { ...base, score: 10, maxScore: 10 }) as Achievement[];
  ...
  const userId2 = await createTestUser("speed-demon-hints");
  const withHints = await database.checkAndGrantAchievements(userId2, { ...base, score: 10, maxScore: 10, hintsUsed: 1 }) as Achievement[];
  assert.deepEqual(withHints.filter((a) => a.id === "speed_demon"), []);

  // With wrong answers — does not grant
  const userId3 = await createTestUser("speed-demon-wrong");
  const imperfect = await database.checkAndGrantAchievements(userId3, { ...base, score: 9, maxScore: 10, hintsUsed: 0 }) as Achievement[];
  assert.deepEqual(imperfect.filter((a) => a.id === "speed_demon"), []);
});

// ─── checkAndGrantAchievements — polyglot ────────────────────────────────────

test("checkAndGrantAchievements grants polyglot when 2+ languages have XP", async () => {
  const userId = await createTestUser("polyglot");
  ...
  await database.recordSession(sessionArgs("spanish"));
  const oneLanguage = await database.checkAndGrantAchievements(userId, checkArgs("spanish", 20)) as Achievement[];
  ...
  await database.recordSession(sessionArgs("italian"));
  const twoLanguages = await database.checkAndGrantAchievements(userId, checkArgs("italian", 40)) as Achievement[];
  assert.ok(twoLanguages.find((a) => a.id === "polyglot"), "polyglot should be granted after 2 languages have XP");
});

// ─── getUserAchievements ──────────────────────────────────────────────────────

test("getUserAchievements returns all earned achievements with correct shape", async () => {
  const userId = await createTestUser("get-achievements");

  // Nothing earned yet
  assert.deepEqual(await database.getUserAchievements(userId), []);

  await database.checkAndGrantAchievements(userId, {
    streak: 7, totalXp: 500, language: "spanish", category: "essentials",
    mastery: 0, hintsUsed: 0, revealedAnswers: 0, score: 5, maxScore: 10
  } satisfies AchievementCheckInput);

  const achievements = await database.getUserAchievements(userId) as Achievement[];
